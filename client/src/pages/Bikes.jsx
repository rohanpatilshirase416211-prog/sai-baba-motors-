...
import { Filter, SlidersHorizontal, ArrowUpDown, Bike, Search } from 'lucide-react';
...
const Bikes = () => {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        const res = await vehicleAPI.getFilters('bike');
        if (res.data.success) {
          setBrands(res.data.data.brands || []);
          setPassingList(res.data.data.passingList || []);
        }
      } catch (err) {
        console.error('Failed to load bike filters:', err);
      }
    };
    fetchFilters();
  }, []);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gold-600 mb-1">
            ...
            <span>Second-Hand Two-Wheelers Marketplace</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-black text-navy-950 font-display">
            Used Bikes & Scooters in Kasba Walve
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Showing {totalVehicles} verified pre-owned motorcycles & scooters with guaranteed paper clearance.
          </p>
        </div>
      ...
      <div className="mb-8 relative max-w-xl">
        ...
        <input
          ...
          placeholder="Search by bike model (e.g. Classic 350, Pulsar, Activa, Splendor)..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 items-start">
        ...
        <div className="md:col-span-3">
          <VehicleGrid
            vehicles={vehicles}
            isLoading={isLoading}
            emptyTitle="No bikes match your criteria"
            emptyDescription="Try adjusting your budget or search for another brand or model."
            onResetFilters={handleResetFilters}
          />
        </div>
      </div>
